...
import com.github.veccvs.djforsenbotkotlin.model.MediaLink
import com.github.veccvs.djforsenbotkotlin.model.Playlist
import com.github.veccvs.djforsenbotkotlin.model.PlaylistItem
...
class SongSchedulerTest {
    ...
    fun `scheduleSong should add random song when playlist is empty and bot is enabled`() {
        ...
        val emptyPlaylist = Playlist(0, false, false, 0, null, mutableListOf())
        val song = Song().apply { 
            id = UUID.randomUUID()
    ...
    fun `scheduleSong should not add song when playlist is not empty`() {
        ...
        val nonEmptyPlaylist = Playlist(0, false, false, 0, null, mutableListOf(playlistItem))
    ...
    fun `changeLastSong should update lastSong when song changes`() {
        ...
        val mediaLink = MediaLink("yt", "videoId")
        val playlistItem = PlaylistItem(1, false, "testuser", mediaLink, "Test Video", 180)
        val playlist = Playlist(0, false, false, 0, null, mutableListOf(playlistItem))
    ...
    fun `changeLastSong should truncate long titles`() {
        ...
        // Assert
        // We can't easily verify the exact LastSong object, so we'll just verify the other interactions
        verify(commandService).sendMessage("#testchannel", "docJAM now playing: " + "A".repeat(50) + "[...]")
        verify(skipCounterService).resetSkipCounter()
    }
    ...
    fun `notifyUsers should notify users who can add videos`() {
        ...
        // Assert
        verify(commandService).sendMessage("#testchannel", "@testuser forsenJam you can add song now! forsenMaxLevel")
