...
class CommandServiceTest {
    ...
    @Mock
    private lateinit var spotifyCommandService: SpotifyCommandService

    @Mock
    private lateinit var playlistCommandService: PlaylistCommandService
    ...
    fun `test commandHandler when user cannot respond to command`() {
        // Given
        val message = ";test"
        `when`(commandParserService.detectCommand(message)).thenReturn("djfors_test")
    ...
    fun `test commandHandler with link command`() {
        ...
        `when`(commandParserService.detectCommand(message)).thenReturn("djfors_link")
        ...
        `when`(commandParserService.parseCommand(message))
            .thenReturn(TwitchCommand("djfors_link", emptyList()))
    ...
    fun `test commandHandler with where command`() {
        ...
        val message = ";where"
        val user = User(testUsername)
        `when`(commandParserService.detectCommand(message)).thenReturn("djfors_where")
        `when`(userRepository.findByUsername(testUsername)).thenReturn(user)
        `when`(timeRestrictionService.canResponseToCommand(testUsername)).thenReturn(true)
        `when`(commandParserService.parseCommand(message))
            .thenReturn(TwitchCommand("djfors_where", emptyList()))
    ...
    fun `test commandHandler with search command when user can add video`() {
        ...
        val user = User(testUsername)
        val twitchCommand = TwitchCommand("djfors_search", listOf("test"))

        `when`(commandParserService.detectCommand(message)).thenReturn("djfors_search")
        `when`(userRepository.findByUsername(testUsername)).thenReturn(user)
        `when`(timeRestrictionService.canResponseToCommand(testUsername)).thenReturn(true)
    ...
    fun `test commandHandler with rg command`() {
        ...
        `when`(userRepository.findByUsername(testUsername)).thenReturn(user)
        `when`(timeRestrictionService.canResponseToCommand(testUsername)).thenReturn(true)
        `when`(commandParserService.parseCommand(message)).thenReturn(
            TwitchCommand(
                ";rg",
                emptyList()
            )
    ...
    fun `test commandHandler with help command`() {
        ...
        verify(messageService)
            .sendMessage(
                testChannel,
                "docJAM @$testUsername Commands: djfors_link, djfors_where, djfors_search, djfors_s, djfors_m, djfors_match, djfors_help, djfors_playlist, djfors_skip, djfors_rg, djfors_when, djfors_undo, djfors_connect, djfors_track, djfors_track stop, djfors_current",
            )
    }
