...
class CytubeDaoTest {
    ...
    inner class TestCytubeDao(restTemplate: RestTemplate, userConfig: UserConfig) : CytubeDao(restTemplate, userConfig) {
    ...
    fun `searchVideos should return list of videos from actual API call`() {
        ...
        val video1Data = LinkedHashMap<String, Any>().apply {
            put("id", "video1")
        ...
        val video2Data = LinkedHashMap<String, Any>().apply {
            put("id", "video2")
        ...
        `when`(restTemplate.getForEntity<List<LinkedHashMap<String, Any>>>(
    ...
    fun `searchVideos should handle non-list thumbnails in actual API call`() {
        ...
        val videoData = LinkedHashMap<String, Any>().apply {
            put("id", "video1")
        ...
        assertEquals("video1", result?.get(0)?.id)
        assertEquals(0, result?.get(0)?.thumbnails?.size) // Should be empty list when thumbnails is not a List
    }
